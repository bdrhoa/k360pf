...
#             "username": incoming_data.get("username"),
#             "accountIsActive": incoming_data.get("account_is_active"),
#         } if incoming_data.get("account_id") else None,
#         "items": [
#             {
...
#                     "width": item.get("width"),
#                     "depth": item.get("depth"),
#                 } if any(k in item for k in ["color", "size", "weight", "height", "width", "depth"]) else None,
#                 "descriptors": item.get("descriptors"),
#                 "id": item.get("item_id"),
#                 "isService": item.get("is_service"),
#             }
...
#                     "paymentToken": transaction["payment"].get("payment_token"),
#                     "bin": transaction["payment"].get("bin"),
#                     "last4": transaction["payment"].get("last4"),
#                 },
#                 "subtotal": str(transaction["subtotal"]),
#                 "orderTotal": str(transaction["order_total"]),
#                 "currency": transaction.get("currency"),
#                 "tax": {
#                     "isTaxable": transaction["tax"]["is_taxable"],
#                     "taxableCountryCode": transaction["tax"].get("taxable_country_code"),
#                     "taxAmount": str(transaction["tax"].get("tax_amount", "0")),
#                     "outOfStateTaxAmount": str(transaction["tax"].get("out_of_state_tax_amount", "0")),
#                 },
#                 "merchantTransactionId": transaction.get("merchant_transaction_id"),
#             }
#             for transaction in incoming_data.get("transactions", [])
#         ] if "transactions" in incoming_data else None,
#         "customFields": incoming_data.get("custom_fields"),
#     }
...
def build_patch_payload(incoming_data: dict) -> dict:
    payload = {
        "merchantOrderId": incoming_data.get("merchantOrderId"),
        "deviceSessionId": incoming_data.get("deviceSessionId"),
        "riskInquiry": incoming_data.get("riskInquiry"),
        "decision": incoming_data.get("decision"),
        "transactions": incoming_data.get("transactions"),
        "fulfillment": incoming_data.get("fulfillment"),
        "shipping": incoming_data.get("shipping"),
    }
...
async def patch_order(order_id: str, payload: dict):
    ...
    url = f"{KOUNT_API_ENDPOINT}{order_id}"
    async with aiohttp.ClientSession() as session:
        try:
...
async def lifespan(app: FastAPI):
    ...
    # Startup logic
    await fetch_or_refresh_token()
    app.state.refresh_task = asyncio.create_task(start_token_refresh_timer())
...
@app.patch("/update-order/{order_id}")
async def update_order(order_id: str, request: Request):
    """
    Handle incoming PATCH requests to update an order in Kount API.
    Args:
        order_id (str): The ID of the order to be updated.
        request (Request): The incoming FastAPI request object.
    Returns:
        JSONResponse: Response from the Kount API.
    Raises:
        HTTPException: If any error occurs during processing.
    """
    try:
        incoming_data = await request.json()
        payload = build_patch_payload(incoming_data)
        response = await patch_order(order_id, payload)
        return JSONResponse(content=response)
    except HTTPException as e:
        raise e
    except Exception as e:
